...
# Traits 
use App\Repositories\PaymentRepository\CalculationTraits;

# Model
use App\Models\Payment;

# Interface
use App\Repositories\PaymentRepository\PaymentInterface;
...
class PaymentRepo implements PaymentInterface
{
   ...
   */
    ...
    {
        return Payment::where('loan_id', $loan_id)->exists();
    }
    ...
    protected function saveNewRepaymentPlan($loan_id, $data)
    {
        ...
        $create = Payment::create($new_data);
        return $create;
    }
   ...
   */
    ...
    {
        return Payment::where('loan_id', $loan_id)->where('payment_status', 'unpaid')->orderBy('payment_no')->first();
    }

    /* get all repayments records that have NOT been paid yet by the loan */
    public function getOutstandingByLoanId($loan_id)
    {
        return Payment::where('loan_id', $loan_id)->where('payment_status', 'unpaid')->get();
    }
    ...
    {
        $over_due = Payment::with('loan')->where('due_date', '<', $this->now)->where('payment_status', 'unpaid')->get();
        return $over_due;
    }
}
